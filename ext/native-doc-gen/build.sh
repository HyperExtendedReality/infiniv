...
[ "$OS" == "Windows_NT" ] && curl -z node.exe -Lo node.exe https://nodejs.org/dist/latest-v22.x/win-x64/node.exe --http1.1
[ "$OS" != "Windows_NT" ] && mkdir /tmp/node && \
	curl -Lo /tmp/node/node.tar.gz https://nodejs.org/dist/latest-v22.x/node-v22.20.0-linux-x64.tar.gz && \
